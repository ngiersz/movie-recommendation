...
class RatingsClient:
    ...
    def get_ratings_json(self):
        df_ratings = self.ratings.reset_index()
        return df_ratings.to_json(orient='index')
    ...
    def add_to_ratings(self, rating):
    ...
    def get_avg_ratings_for_genres(self):
        ...
        DFAllUsersRatedMoviesWithMovieGenres = pd.DataFrame()
        for genre in genres:
            genre_ratings = self.ratings[self.ratings[genre] == 1]
            DFAllUsersRatedMoviesWithMovieGenres = DFAllUsersRatedMoviesWithMovieGenres.append(
                pd.Series(genre_ratings.rating.mean(), index=[genre]), ignore_index=True)
        print('all users:')
        print(DFAllUsersRatedMoviesWithMovieGenres.sum())
        return DFAllUsersRatedMoviesWithMovieGenres.sum()

    # def is_genre_times_rating(self, row, genres):
    #     genres_ratings = pd.Series(index=genres)
    #     for genre in genres:
    #         genres_ratings[genre] = row.rating * row.genre

    # # zad 5
    # def get_ratings_for_all_movies_unbiased(self):
    #     ratings_and_genres = self.create_merged_ratings_and_genres()
    #     print(ratings_and_genres.head())
    #     genres = self.get_genres_column_names(ratings_and_genres)
    #     ratings_and_genres.apply(self.is_genre_times_rating(genres), axis=1)
    #     print(ratings_and_genres.head())
    #     return ratings_and_genres

    # zad 6
    def get_avg_user_ratings_for_genres(self, user_id):
        genres = self.get_genres_column_names(self.ratings)
        user_ratings = self.ratings[self.ratings.userID == user_id]
        DFUserRatedMoviesWithMovieGenres = pd.DataFrame()
        for genre in genres:
            genre_ratings = user_ratings[user_ratings[genre] == 1]
            DFUserRatedMoviesWithMovieGenres = DFUserRatedMoviesWithMovieGenres.append(
                pd.Series(genre_ratings.rating.mean(), index=[genre]), ignore_index=True)
        DFUserRatedMoviesWithMovieGenres = DFUserRatedMoviesWithMovieGenres.sum()
        print('biased: ')
        print(DFUserRatedMoviesWithMovieGenres)
        return DFUserRatedMoviesWithMovieGenres

    # zad 6
    def DFUserRatedMoviesWithMovieGenres_unbiased(self, user_id):
...
if __name__ == "__main__":
    ...
    unbiased = cl.DFUserRatedMoviesWithMovieGenres_unbiased(75)
    print('unbiased:')
    print(unbiased)

    # print(cl.get_ratings_for_all_movies_unbiased().head())
